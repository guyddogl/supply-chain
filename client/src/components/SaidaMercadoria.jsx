...
export default function saidaMercadoria() {
  const [show, setShow] = useState(false);

  const [isLoading, setIsLoading] = useState(false);

  // const [qntdDisponivel, setQntdDisponivel] = useState(null);
  ...
  const handleFormInputs = (event) => {
    const { target } = event;
    if (target.name === 'mercadoria') checkAvailability(target.value);
  ...
  const addNewMercadoria = async (event) => {
    ...
    await Axios.post('/saidas', inputsForm)
      .then(() => {
        showToast('success', 'Saída de mercadoria concluída');
        setIsLoading(false);
        setUpdate(!update);
  ...
  return (
    <>
      ...
      >
        <Modal.Header>
          <Modal.Title>
            <i className="fa-solid fa-circle-minus text-warning me-2" />
            Saída de Mercadoria
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <form onSubmit={addNewMercadoria}>
              ...
              <div className="col-12 col-lg-5">
                <label htmlFor="quantidade" className="form-label" style={{ width: '100%' }}>
                  <input
                    ...
                    required
                  />
                  <span className="d-block valid-feedback ms-1">
            ...
            {isLoading ? (
              <div className="d-grid gap-2 d-flex justify-content-md-end mt-3">
                ...
                <button
                  type="button"
                  className="btn btn-md btn-warning mx-1 my-3"
                  style={{ minWidth: '140px' }}
                  disabled
                >
